...
class ModelArguments:
    ...
    ordinal_classification: bool = field(
        default=False,
        metadata={"help": "ordinal_classification, dist and int2label must be set"}
    )
    #dist: str = field(
    #    default=None,
    ...
    oll_alpha: float = field(
        default=1.5,
        metadata={"help": "oll_alpha for ordinal_classification loss"}
    )
...
def main():
    ...
    if model_args.ordinal_classification:
        ...
        model.config.dist_matrix = generate_dist_matrix(num_labels)
    ...
    def compute_metrics(p: EvalPrediction):
        ...
        for m in metric:
            ...
            if hasattr(r[m], '__iter__'):
                ...
                avg = "macro"
                result[f"{m} ({avg})"] = metric[m].compute(predictions=preds, references=p.label_ids, average=avg)[m]
            else:
                result.update(r)
    ...
    if model_args.apply_class_weights and not is_regression:
        ...
        class OLLTrainer(Trainer):
            def compute_loss(self, model, inputs, return_outputs=False):
                ...
                dist_matrix = model.config.dist_matrix
