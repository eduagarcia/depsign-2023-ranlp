...
class ModelArguments:
    ...
    early_stopping_patience: Optional[int] = field(
        default=None, metadata={"help": "early_stopping_patience"}
    )
    early_stopping_threshold: float = field(
        default=0.0, metadata={"help": "early_stopping_threshold"}
    )
    apply_class_weights: bool = field(
        default=False, metadata={"help": "apply_class_weights"}
    )

    regression: bool = field(
        default=False, metadata={"help": "Train as Regression"}
    )
...
def main():
    ...
    # Labels
    if data_args.task_name is not None:
        is_regression = model_args.regression
        if not is_regression:
            ...
            label_unmap = {value: key for key, value in label_map.items()}
    else:
    ...
    if data_args.task_name is not None:
        ...
        if (
            ...
            if "pid" in non_label_column_names:
                non_label_column_names.remove("pid")
    ...
    def preprocess_function(examples):
        ...
        if is_regression:
            result["label"] = [float(l) for l in result["label"]]
    ...
    def compute_metrics(p: EvalPrediction):
        ...
        result = dict()
        for m in metric:
            kwargs = {"average": None} if m != "accuracy" else {}

            print(p.label_ids)
            r = metric[m].compute(predictions=preds, references=p.label_ids, **kwargs)

            if is_regression:
                id2label = label_unmap
            else:
                id2label = model.config.id2label

            if hasattr(r[m], "__iter__"):
                for idx, label in id2label.items():
                    result[f"{m} ({label})"] = r[m][idx]

                avg = "macro"
                result[f"{m} ({avg})"] = metric[m].compute(
                    predictions=preds, references=p.label_ids, average=avg
                )[m]
            else:
                result.update(r)
    ...
    if model_args.apply_class_weights and not is_regression:
        ...
        class_weights = torch.tensor(
            computed_class_weights, dtype=torch.float, device=training_args.device
        )
